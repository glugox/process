...
class Save extends \Glugox\Process\Controller\Adminhtml\Process
{
        ...
        if ($this->getRequest()->getPostValue()) {
            try {
                ...
                if ($id) {
                    $model->load($id);
                    if ($id != $model->getId()) {
                        throw new \Magento\Framework\Exception\LocalizedException(__('The wrong process is specified.'));
                    }
                }
                $model->setData($data);
                ...
                $this->messageManager->addError(
                    __('Something went wrong while saving the process data. Please review the error log.')
                );
